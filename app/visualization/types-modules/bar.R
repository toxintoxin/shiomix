barUI <- function(type) {
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      condition = "input.position == '无边框并紧贴' || input.position == '有边框有空隙'",
      div(style = "display: flex;",
        div(style = "width: 26%; height: 85px;", prettySwitch(ns("errorbar"), "errorbar", value = TRUE)),
        conditionalPanel(
          ns = ns,
      ...
      div(style = "display: flex;",
        div(style = "width: 26%; height: 85px;", prettySwitch(ns("jitter"), "数据点", value = TRUE)),
        conditionalPanel(
          ns = ns,
...
barServer <- function(id, data_ready) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$x, {
      ...
      if (input$x == "group") {
        x_multi_choices <- unique(df$group)
      } else if (input$x == "var") {
        x_multi_choices <- unique(df$var)
      }

      if (input$x == "group") {
        fill_multi_choices <- unique(df$var)
      } else if (input$x == "var") {
        fill_multi_choices <- unique(df$group)
      }
    ...
    output$aes_fill_plan <- renderUI({
      if (input$aes_fill == "预设") {
        ...
        colorPickr_list <- map(input$x_multi, function(label) {
          div(style = "width: 14%;", colorPickr(inputId = paste0("aes_fill_", gsub(":", "_", label)), label = label, selected = "#000000"))
        })
    ...
    df_summary <- df %>% group_by(group, var) %>% summarise(
      count = n(), Mean = mean(value), SD = sd(value), SEM = sd(value) / sqrt(n())
    )
    output$data_summary <- renderTable({df_summary})

    reactive({


      # 只绘制选中的，并映射
      if (input$x == "group") {p <- df %>% filter(group %in% input$x_multi) %>% filter(var %in% input$fill_multi) %>% ggplot(aes(x = group, y = value, fill = var))}
      else if (input$x == "var") {p <- df %>% filter(var %in% input$x_multi) %>% filter(group %in% input$fill_multi) %>% ggplot(aes(x = var, y = value, fill = group))}
