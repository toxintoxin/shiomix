ggthemeUI <- function(id) {
  ...
  tagList(
    layout_columns(
      ...
      numericInput(ns("axis.title.x_size"), label = NULL, value = 11, min = 8, max = 22),
      checkboxGroupButtons(ns("axis.title.x_face"), label = NULL, c(`<i class="fa fa-bold"></i>` = "B", `<i class="fa fa-italic"></i>`= "I"), justified = TRUE),
      colorPickr(ns("axis.title.x_colour"), label = NULL, selected = "#000000"),
    ...
    layout_columns(
      ...
      numericInput(ns("axis.title.y_size"), label = NULL, value = 11, min = 8, max = 22),
      checkboxGroupButtons(ns("axis.title.y_face"), label = NULL, c(`<i class="fa fa-bold"></i>` = "B", `<i class="fa fa-italic"></i>`= "I"), justified = TRUE),
      colorPickr(ns("axis.title.y_colour"), label = NULL, selected = "#000000"),
    ...
    layout_columns(
      ...
      numericInput(ns("plot.title_size"), label = NULL, value = 13.2, min = 8, max = 22),
      checkboxGroupButtons(ns("plot.title_face"), label = NULL, choices = c(`<i class="fa fa-bold"></i>` = "B", `<i class="fa fa-italic"></i>`= "I"), justified = TRUE),
      colorPickr(ns("plot.title_colour"), label = NULL, selected = "#000000"),
    ...
    layout_columns(
      ...
      numericInput(ns("plot.subtitle_size"), label = NULL, value = 11, min = 8, max = 22),
      checkboxGroupButtons(ns("plot.subtitle_face"), label = NULL, c(`<i class="fa fa-bold"></i>` = "B", `<i class="fa fa-italic"></i>`= "I"), justified = TRUE),
      colorPickr(ns("plot.subtitle_colour"), label = NULL, selected = "#000000"),
    ...
    layout_columns(
      ...
      numericInput(ns("plot.caption_size"), label = NULL, value = 8.8, min = 8, max = 22),
      checkboxGroupButtons(ns("plot.caption_face"), label = NULL, c(`<i class="fa fa-bold"></i>` = "B", `<i class="fa fa-italic"></i>`= "I"), justified = TRUE),
      colorPickr(ns("plot.caption_colour"), label = NULL, selected = "#000000"),
    ...
    layout_columns(
      ...
      numericInput(ns("plot.tag_size"), label = NULL, value = 13.2, min = 8, max = 22),
      checkboxGroupButtons(ns("plot.tag_face"), label = NULL, c(`<i class="fa fa-bold"></i>` = "B", `<i class="fa fa-italic"></i>`= "I"), justified = TRUE),
      colorPickr(ns("plot.tag_colour"), label = NULL, selected = "#000000"),
    ...
    layout_columns(
      layout_columns(col_widths = c(12, 12),
        ...
        layout_columns(id = ns("panel.grid.major_x_custom"),
          ...
          colorPickr(ns("panel.grid.major.x_colour"), label = NULL, selected = "#EBEBEB")
        )
      ),
      layout_columns(col_widths = c(12, 12),
        ...
        layout_columns(id = ns("panel.grid.minor_x_custom"),
          ...
          colorPickr(ns("panel.grid.minor.x_colour"), label = NULL, selected = "#EBEBEB")
        )
      )
    ),
    layout_columns(
      layout_columns(col_widths = c(12, 12),
        ...
        layout_columns(id = ns("panel.grid.major_y_custom"),
          ...
          colorPickr(ns("panel.grid.major.y_colour"), label = NULL, selected = "#EBEBEB")
        )
      ),
      layout_columns(col_widths = c(12, 12),
        ...
        layout_columns(id = ns("panel.grid.minor_y_custom"),
          ...
          colorPickr(ns("panel.grid.minor.y_colour"), label = NULL, selected = "#EBEBEB")
        )
      )
    ),
    layout_columns(
      layout_columns(col_widths = c(12, 4),
        input_switch(ns("panel.background"), label = "绘图区背景", value = FALSE) %>% tooltip("默认是透明的，打开后会产生额外的矢量元素供你选择填充颜色"),
        layout_columns(id = ns("panel.background_custom"), colorPickr(ns("panel.background_fill"), label = NULL, selected = "#FFFFFF"))
      ),
      layout_columns(col_widths = c(12, 12),
    ...
    layout_columns(
      layout_columns(col_widths = c(12, 4),
        input_switch(ns("plot.background"), label = "图片背景", value = FALSE) %>% tooltip("默认是透明的，打开后会产生额外的矢量元素供你选择填充颜色"),
  ...
  moduleServer(id, function(input, output, session) {
    p_return <- reactive({
      p <- ggobj() + theme(
        ...
        axis.title.x = if(input$axis.title.x_size <= 0) {element_blank()} else{element_text(family = input$axis.title.x_family, face = face2(input$axis.title.x_face), colour = input$axis.title.x_colour, size = input$axis.title.x_size, hjust = input$axis.title.x_hjust, angle = input$axis.title.x_angle)},
        axis.title.y = if(input$axis.title.y_size <= 0) {element_blank()} else{element_text(family = input$axis.title.y_family, face = face2(input$axis.title.y_face), colour = input$axis.title.y_colour, size = input$axis.title.y_size, hjust = input$axis.title.y_hjust, angle = input$axis.title.y_angle)},
        plot.title = if(input$plot.title_size <= 0) {element_blank()} else{element_text(family = input$plot.title_family, face = face2(input$plot.title_face), colour = input$plot.title_colour, size = input$plot.title_size, hjust = input$plot.title_hjust, angle = input$plot.title_angle)},
        plot.subtitle = if(input$plot.subtitle_size <= 0) {element_blank()} else{element_text(family = input$plot.subtitle_family, face = face2(input$plot.subtitle_face), colour = input$plot.subtitle_colour, size = input$plot.subtitle_size, hjust = input$plot.subtitle_hjust, angle = input$plot.subtitle_angle)},
        plot.caption = if(input$plot.caption_size <= 0) {element_blank()} else{element_text(family = input$plot.caption_family, face = face2(input$plot.caption_face), colour = input$plot.caption_colour, size = input$plot.caption_size, hjust = input$plot.caption_hjust, angle = input$plot.caption_angle)},
        plot.tag = if(input$plot.tag_size <= 0) {element_blank()} else{element_text(family = input$plot.tag_family, face = face2(input$plot.tag_face), colour = input$plot.tag_colour, size = input$plot.tag_size, hjust = input$plot.tag_hjust, angle = input$plot.tag_angle)},
      ...
      #   # axis.text.x = if(input$axis_text_x == FALSE || input$axis.text.x_size <= 0) {element_blank()} else{element_text(family = input$axis.text.x_family, face = face2(input$axis.text.x_face), colour = input$axis.text.x_colour, size = input$axis.text.x_size, angle = input$axis.text.x_angle)},
      #   # axis.text.y = if(input$axis_text_y == FALSE || input$axis.text.y_size <= 0) {element_blank()} else{element_text(family = input$axis.text.y_family, face = face2(input$axis.text.y_face), colour = input$axis.text.y_colour, size = input$axis.text.y_size, angle = input$axis.text.y_angle)},
      ...
      # )
      # if(input$axis_ticks_x == TRUE && input$axis.ticks.x_linewidth > 0 && input$axis.ticks.length.x_length != 0) {p <- p + theme(axis.ticks.length.x = unit(input$axis.ticks.length.x_length, "mm"))} else { }
      # if(input$axis_ticks_y == TRUE && input$axis.ticks.y_linewidth > 0 && input$axis.ticks.length.y_length != 0) {p <- p + theme(axis.ticks.length.y = unit(input$axis.ticks.length.y_length, "mm"))} else { }
      return(p)
    })
    return(p_return)
  })
